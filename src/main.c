...
void car_gpio_init(void) {
    ...
    g.GPIO_Pin = LEFT_A_PIN; GPIO_Init(LEFT_A_PORT, &g);
    g.GPIO_Pin = LEFT_B_PIN; GPIO_Init(LEFT_B_PORT, &g);
    g.GPIO_Pin = RIGHT_A_PIN; GPIO_Init(RIGHT_A_PORT, &g);
    g.GPIO_Pin = RIGHT_B_PIN; GPIO_Init(RIGHT_B_PORT, &g);
...
void sensor_gpio_init(void) {
    GPIO_InitTypeDef g;
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOG | RCC_AHB1Periph_GPIOB, ENABLE); // 使能GPIOG和GPIOB
...
int main() {
    ...
    while(1) {
        ...
        // 没有按键按下 -> 执行循迹逻辑
            ...
            else if(left == 1 && right == 0) { // 左探头在黑色，右探头在白色 -> 左转
                // 使用增强版左转，提高左转灵敏度
                Car_SharpTurnLeft();
                Mdelay_Lib(35);  // 增加左转时间，与右转保持一致
            }
            else if(left == 0 && right == 1) { // 左探头在白色，右探头在黑色 -> 右转
                // 使用增强版右转，提高右转灵敏度
                Car_SharpTurnRight();
